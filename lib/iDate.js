...
iDate = function (val) {
    ...
    if (val == undefined) {
        ...
        typeof val === 'number' || val === 'today' || val === 'tomorrow' || val === 'yesterday' || val === 'now'
    ) {
         ...
         * parse date
         */
        for (i = 0, len = this.parsers.date.length; i < len && !parsedDate && this.parsers.date[i]; i++) {
         ...
         * parse time
         */
        for (i = 0, len = this.parsers.time.length; i < len && !parsedTime; i++) {
        ...
        if (
            ...
            for (attr in this.attributes) {
                if (this.attributes[attr].val != null) {
    ...
    parsers:           {
        ...
        date: [
            ...
            function (dateStr) {
                ...
                var reg = (/([0-9]{4})([0-1][0-9])([0-3][0-9])T([0-9]{2}):([0-9]{2}):([0-9]{2})/);
                var arr = reg.exec(dateStr);

                if (arr) {
                    this.set('date', (+arr[3]));
                    this.set('month', (+arr[2]));
                    this.set('year', (+arr[1]));
                    this.set('hours', (+arr[4]));
                    this.set('minutes', (+arr[5]));
                    this.set('seconds', (+arr[6]));
